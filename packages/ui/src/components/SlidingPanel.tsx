...
const SlidingPanelContext = createContext<SlidingPanelContextProps | undefined>(undefined)
...
function Backdrop({ className, ...rest }: HTMLAttributes<HTMLDivElement>) {
  const ctx = useContext(SlidingPanelContext)
  if (!ctx) throw new Error('SlidingPanel.Backdrop must be used within SlidingPanel.Wrapper')
...
function Close({ children, className, ...rest }: { children?: ReactNode } & HTMLAttributes<HTMLButtonElement>) {
  const ctx = useContext(SlidingPanelContext)
  if (!ctx) throw new Error('SlidingPanel.Close must be used within SlidingPanel.Wrapper')
  return (
    <button
      onClick={ctx.onClose}
